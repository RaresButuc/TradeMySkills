...
export default function ChangePassword(){
    ...
    const unlockSaveButton = () =>{
        if((newPassword.current.value === confirmNewPassword.current.value) && newPassword.current.value.length > 0){
            setActive(true);
        }else{
            setActive(false);
        }
 ...
 console.log(email);
    const handleSubmit = async (e) => {
        ...
        const confirmPasswordForm = formData.get("confirm-password");
